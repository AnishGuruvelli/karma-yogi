...
export async function loginAs(page: Page, email = TEST_EMAIL, password = TEST_PASSWORD) {
  await page.goto("/");
  await page.waitForSelector('[id="auth-email"]', { timeout: 15_000 });
  ...
  if (!navShown) {
    ...
    const stillOnAuth = await page.locator('[id="auth-email"]').isVisible().catch(() => false);
    if (!stillOnAuth) {
      // Mid-transition — wait a bit more for nav
      await page.waitForSelector("nav", { timeout: 10_000 });
      return;
    }
    // Auth page still visible: account doesn't exist yet — register it
    await page.locator('button[type="button"]:has-text("Create an account")').click();
    await page.waitForSelector('[id="auth-fullname"]', { timeout: 8_000 });
    ...
    await page.click('button[type="submit"]');
    await page.waitForSelector("nav", { timeout: 22_000 });
  }
}
